...
3. **Cómo te acompaña Saiki (Bento Grid)**: features (límite de capacidad, auditoría mensual, cierre semestral, reprogramar/repriorizar) y los 5 Estados de Resiliencia (Optimismo, Tranquilidad, Certeza, Fortaleza, Neutralidad) combinados en una sola grilla tipo bento — tiles de distinto tamaño, con los 5 tiles de resiliencia usando su color de estado como fondo completo (no solo un acento), y los tiles de features en tono neutro. Cada definición de resiliencia usa el texto en segunda persona provisto por el autor, tal cual.
4. **Capturas de pantalla**: 3 capturas reales de la app (aportadas por el usuario en `imagenes de saiki/`), elegidas para mostrar variedad de vista (tablero/calendario) y tema (claro/oscuro).
